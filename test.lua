...
-- Fungsi Auto Fishing V5 - PASTI WORK dari Custom GUI
local function autoFishingV5Loop()
    while autoFishingV5Enabled do
        local ok, err = pcall(function()
            fishingV5Active = true
            updateStatus("🎣 Status: Fishing V5", Color3.fromRGB(100, 255, 100))
...
local function StartFishingV5()
    ...
    print("🎣 FISHING V5 STARTED - COPAS DARI CUSTOM GUI")
    ...
    Rayfield:Notify({
        ...
        Content = "System dari Custom GUI - PASTI WORK!",
        Duration = 5,
        Image = 4483362458
    })
end
...
-- Di bagian Main Tab, tambahkan ini:

-- Fishing V5 (Dari Custom GUI yang work)
MainTab:CreateToggle({
    Name = "🚀 Fishing V5 (PASTI WORK - dari Custom GUI)",
    CurrentValue = false,
    Callback = function(Value)
        if Value then
            -- Matikan sistem lain
...
task.spawn(function()
    while task.wait(1) do
        ...
        if autoFishingV5Enabled then
            statusText = "V5 Status: 🟢 ACTIVE - PASTI WORK!"
        else
