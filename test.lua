        ...
        for attrName, attrValue in pairs(equippedRod:GetAttributes()) do
            ...
            if nameLower:find("reel") and nameLower:find("faster") then
                ...
                if speedBonus > 0 then
                    speedMultiplier = math.max(0.3, 1.0 - (speedBonus / 100)) -- Lebih agresif
...
-- TARIK SEBELUM TANDA SERU MUNCUL!
local function setupPredictivePullSystem()
    ...
    if textEffectRemote then
        ExclaimConnection = textEffectRemote.OnClientEvent:Connect(function(data)
            if data and data.TextData and data.TextData.EffectType == "Exclaim" and EnhanceFishing.Enabled then
                print("🎯 Exconfirm: Predictive pull worked!")
...
-- 🚀 ENHANCED FISHING CORE LOOP DENGAN PREDICTIVE PULL
local function enhanceFishingLoop()
    EnhanceFishing.SuccessPattern = {}
    local consecutivePulls = 0
    ...
    while EnhanceFishing.Enabled do
        local success, err = pcall(function()
            ...
            task.wait(0.2 * EnhanceFishing.SpeedMultiplier) -- Lebih cepat
            
            -- Start fishing cast
            local timestamp = workspace:GetServerTimeNow()
            rodRemote:InvokeServer(timestamp)
            task.wait(0.15 * EnhanceFishing.SpeedMultiplier) -- Lebih cepat
            
            -- Perfect cast coordinates
            local x, y
            if EnhanceFishing.PerfectCast then
                x = -0.7499996 + (math.random(-10, 10) / 10000000) -- Lebih presisi
                y = 1 + (math.random(-10, 10) / 10000000)
            else
            ...
            -- ⚡ PREDICTIVE PULL TIMING ⚡
            local baseWaitTime = 0.8 * EnhanceFishing.SpeedMultiplier -- Waktu dasar
            local variance = math.random(5, 15) / 100 -- Variasi random
            
            -- Adaptive timing berdasarkan success rate
            if #EnhanceFishing.SuccessPattern > 2 then
                local successCount = 0
                for _, s in ipairs(EnhanceFishing.SuccessPattern) do
                    if s then successCount = successCount + 1 end
                end
                local successRate = successCount / #EnhanceFishing.SuccessPattern
                
                if successRate > 0.7 then
                    baseWaitTime = baseWaitTime * 0.7 -- Lebih cepat jika success rate tinggi
                elseif successRate < 0.3 then
                    baseWaitTime = baseWaitTime * 1.3 -- Lebih lambat jika success rate rendah
                end
            end
            
            local predictiveWait = baseWaitTime + variance
            
            -- ⚡ PHASED PULL SYSTEM ⚡
            local startTime = tick()
            local hasPulled = false
            
            -- Phase 1: Early Pull Attempt (SEBELUM exclaim)
            local earlyPullTime = predictiveWait * 0.6 -- 60% dari waktu normal
            task.wait(earlyPullTime)
            
            if not ExclaimDetected and not hasPulled then
                -- PREDICTIVE PULL - Tarik sebelum exclaim muncul!
                print("🚀 PREDICTIVE PULL - Early attempt!")
                for i = 1, 3 do
                    pcall(function()
                ...
                hasPulled = true
                consecutivePulls = consecutivePulls + 1
            end
            
            -- Phase 2: Monitor for Exclaim dengan interval SANGAT CEPAT
            local monitorStart = tick()
            while (tick() - monitorStart) < (predictiveWait * 0.4) and not ExclaimDetected do
                task.wait(0.001) -- 1ms monitoring - SUPER FAST
                
                -- Instant pull jika exclaim terdeteksi
                if ExclaimDetected and not hasPulled then
                    print("⚡ INSTANT PULL - Exclaim detected!")
                    for i = 1, 8 do
                        pcall(function()
                            finishRemote:FireServer(true)
                        end)
                        if i <= 3 then
                            task.wait(0.0005) -- Ultra ultra fast untuk 3 pertama
                        else
                            task.wait(0.001)
                        end
                    end
            ...
            -- Phase 3: Final Guaranteed Pull
            if not hasPulled then
                print("⏱️ GUARANTEED PULL - Timeout backup")
                for i = 1, 5 do
                    pcall(function()
                        finishRemote:FireServer(true)
                    end)
            ...
            -- Update success pattern berdasarkan apakah exclaim terdeteksi
            table.insert(EnhanceFishing.SuccessPattern, ExclaimDetected)
            
            -- Cleanup pattern array
            if #EnhanceFishing.SuccessPattern > 8 then
            ...
            local timeSinceLastPull = currentTime - lastPullTime
            lastPullTime = currentTime
            
            local baseCooldown = math.random(5, 12) / 100 * EnhanceFishing.SpeedMultiplier
            
            -- Adjust cooldown berdasarkan consecutive pulls
            if consecutivePulls > 3 then
                baseCooldown = baseCooldown * 1.5 -- Slow down jika terlalu banyak pull beruntun
                consecutivePulls = 0
            elseif ExclaimDetected then
                baseCooldown = baseCooldown * 0.8 -- Lebih cepat jika berhasil
            end
            ...
            if #EnhanceFishing.SuccessPattern > 0 then
                ...
                for _, success in ipairs(EnhanceFishing.SuccessPattern) do
                    if success then recentSuccess = recentSuccess + 1 end
                end
                local successRate = recentSuccess / #EnhanceFishing.SuccessPattern
                
                if successRate > 0.8 then
                    baseCooldown = baseCooldown * 0.7 -- Super fast jika success rate tinggi
                elseif successRate < 0.2 then
                    baseCooldown = baseCooldown * 1.4 -- Slow down jika gagal terus
                end
            end
            
            task.wait(math.max(0.03, baseCooldown)) -- Minimum 0.03 detik
...
local function monitorFishThreshold()
    task.spawn(function()
        while EnhanceFishing.Enabled do
            ...
            task.wait(0.3) -- Check lebih sering
        end
    end)
end
...
local function startEnhanceFishing()
    ...
    -- ⚡ SETUP PREDICTIVE PULL SYSTEM ⚡
    setupPredictivePullSystem()
    
    task.spawn(enhanceFishingLoop)
    
    Rayfield:Notify({
        Title = "🚀 PREDICTIVE FISHING Started",
        Content = "Tarik SEBELUM tanda seru muncul! Ultra-fast response!",
        Duration = 4,
        Image = 4483362458
    })
    
    print("🚀 PREDICTIVE PULL SYSTEM ACTIVATED!")
    print("⚡ Phase 1: Early pull attempt (60% timing)")
    print("🎯 Phase 2: 1ms monitoring interval") 
    print("💥 Phase 3: Guaranteed backup pull")
    print("🔄 Adaptive timing berdasarkan success rate")
end
...
local function stopEnhanceFishing()
    ...
    Rayfield:Notify({
        ...
        Content = "Predictive fishing system disabled",
        Duration = 3
    })
end
