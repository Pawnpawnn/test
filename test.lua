...
local function JoinBoostedServer()
    Rayfield:Notify({
        Title = "Searching Boosted Servers",
        Content = "Scanning servers for active boosts...",
        Duration = 3,
        Image = 4483362458
    })
    
    local placeId = game.PlaceId
    local boostedServers = {}
    local cursor = ""
    ...
    repeat
        ...
        if success and result and result.data then
            for _, server in pairs(result.data) do
                -- Cek apakah server punya boost aktif dan masih ada slot
                if server.playing < server.maxPlayers and server.id ~= game.JobId then
                    -- playerTokens menunjukkan jumlah boost (biasanya dari Roblox Premium)
                    if server.playerTokens and #server.playerTokens > 0 then
                        table.insert(boostedServers, {
                            id = server.id,
                            boostCount = #server.playerTokens,
    ...
    until not cursor or #boostedServers >= 10 -- Ambil minimal 10 server atau sampai habis
    
    if #boostedServers > 0 then
        -- Sort berdasarkan jumlah boost (dari tertinggi ke terendah)
        table.sort(boostedServers, function(a, b)
            return a.boostCount > b.boostCount
        end)
        
        local targetServer = boostedServers[1]
        
        Rayfield:Notify({
            Title = "Boosted Server Found!",
            Content = string.format("Joining server with %d boost(s)! (%d/%d players)", 
                targetServer.boostCount, targetServer.playing, targetServer.maxPlayers),
            Duration = 5,
            Image = 4483362458
        })
        ...
        NotifyError("No Boosted Servers", "Could not find any servers with active boosts. Try regular Server Hop instead.")
    end
end
...
MiscTab:CreateButton({
    Title = "🔥 Join Boosted Server",
    Name = "🔥 Join Boosted Server",
    Content = "Join server with active boost",
    Callback = function()
        JoinBoostedServer()
    end,
})
