...
-- Fungsi untuk save data activation ke file
local function KeySystemSaveToFile(data)
    if writefile then
        local success, err = pcall(function()
            writefile("codepik_permanent_keys.json", KeySystemHttpService:JSONEncode(data))
        end)
        return success
    end
    return false
...
-- Fungsi untuk load data activation dari file
local function KeySystemLoadFromFile()
    if readfile and isfile then
        if isfile("codepik_permanent_keys.json") then
            local success, data = pcall(function()
                return KeySystemHttpService:JSONDecode(readfile("codepik_permanent_keys.json"))
            end)
...
-- Validasi key permanent
local function KeySystemValidatePermanentKey(key)
    ...
    if activationData then
        ...
        if activationData.hwid == KeySystemCurrentHWID then
            return true, "✅ Key validated (Permanent)"
        else
            return false, "❌ Key used on different device: " .. string.sub(activationData.hwid, 1, 8) .. "..."
        end
    else
        ...
        for k, data in pairs(KeySystemActivations) do
            if data.hwid == KeySystemCurrentHWID then
                return false, "❌ Device already registered with key: " .. string.sub(k, 1, 8) .. "..."
            end
        end
        ...
        return true, "✅ Key activated! (Permanent License)"
    end
end

-- Cek existing activation
local function KeySystemCheckPermanentActivation()
...
KeySystemMainFrame.Size = UDim2.new(0, 450, 0, 350)
KeySystemMainFrame.Position = UDim2.new(0.5, -225, 0.5, -175)
...
KeySystemTitle.Text = "🔑 Codepik Premium - Permanent License"
...
KeySystemHWIDLabel.Position = UDim2.new(0.05, 0, 0.18, 0)
KeySystemHWIDLabel.BackgroundTransparency = 1
KeySystemHWIDLabel.Text = "Device ID: " .. KeySystemCurrentHWID
...
KeySystemInfoLabel.Position = UDim2.new(0.05, 0, 0.25, 0)
...
KeySystemKeyBox.PlaceholderText = "Enter key format"
...
KeySystemSubmitBtn.Position = UDim2.new(0.2, 0, 0.62, 0)
...
KeySystemStatusMsg.Size = UDim2.new(0.8, 0, 0, 60)
KeySystemStatusMsg.Position = UDim2.new(0.1, 0, 0.78, 0)
...
local function KeySystemLoadMainScript()
    ...
    -- [SCRIPT UTAMA MASUK DI SINI]
    -- ... (script utama yang lama)
...
KeySystemSubmitBtn.MouseButton1Click:Connect(function()
    ...
    local isValid, message = KeySystemValidatePermanentKey(key)
...
local KeySystemHasActivation, KeySystemActivatedKey = KeySystemCheckPermanentActivation()
if KeySystemHasActivation then
    KeySystemStatusMsg.Text = "✅ Permanent License Active!\nKey: " .. string.sub(KeySystemActivatedKey, 1, 12) .. "...\nNo Expiry • Enjoy!"
    KeySystemStatusMsg.TextColor3 = Color3.fromRGB(100, 255, 100)
    wait(3)
    KeySystemLoadMainScript()
else
    KeySystemStatusMsg.Text = "🔑 Codepik Key 1 device system"
    KeySystemStatusMsg.TextColor3 = Color3.fromRGB(255, 255, 255)
end

warn("🔑 Permanent Key System Loaded")
warn("📱 HWID: " .. KeySystemCurrentHWID)
warn("⭐ License: Permanent • 1 Device = 1 Key")