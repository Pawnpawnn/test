...
local function setupRemotes()
   ...
   -- Trade Remote Setup (TAMBAHKAN INI)
...
-- ========== TRADE SYSTEM ===========
-- ===================================
-- TARUH SECTION INI SETELAH setupAnimations() FUNCTION (SEKITAR BARIS 150-200)
...
local function getInventoryValue()
    ...
    pcall(function()
        ...
        if not Replion then 
            ...
            Replion = game:GetService("ReplicatedStorage"):FindFirstChild("Packages")
            if Replion then
                Replion = Replion:FindFirstChild("Replion")
            end
        end
        ...
        if success2 and type(items) == "table" then
            for _, item in ipairs(items) do
                if not item.Favorited and item.Value then
...
local function getFishToTrade(targetValue)
    ...
    pcall(function()
        ...
        if success2 and type(items) == "table" then
            -- Sort items by value (lowest first untuk efisiensi)
            table.sort(items, function(a, b)
                return (a.Value or 0) < (b.Value or 0)
            end)
            
            for _, item in ipairs(items) do
                if not item.Favorited and item.Value and currentValue < targetValue then
                    table.insert(fishList, {
                        UUID = item.UUID,
                        Value = item.Value,
                        Id = item.Id,
                        Name = item.Id or "Unknown Fish"
                    })
...
local function updateTradeProgress()
    if TradeProgressLabel then
        local progressText = string.format(
            ...
            tradeSuccessCount > 0 and (tradeSuccessCount / (tradeSuccessCount + tradeFailedCount) * 100) or 0
        )
...
local function executeTrade()
    ...
    if not targetPlayer then
        Rayfield:Notify({
            ...
            Content = selectedTradePlayer .. " is not in the server!",
            Duration = 3
        })
    ...
    Rayfield:Notify({
        Title = "📤 Trading...",
        Content = string.format("Sending %d fish worth %s", #fishList, formatCurrency(totalValue)),
        Duration = 3,
        Image = 4483362458
    })
    
    -- Execute trade dengan error handling
    local success, err = pcall(function()
        -- Cari trade remote
        if not tradeRemote then
        ...
        if tradeRemote then
            -- Format 1: Send list of UUIDs
            local uuidList = {}
            for _, fish in ipairs(fishList) do
                table.insert(uuidList, fish.UUID)
            end
            
            -- Try multiple trade formats
            local tradeSuccess = false
            
            -- Format 1: Standard trade request
            pcall(function()
                tradeRemote:InvokeServer(targetPlayer.UserId, uuidList)
                tradeSuccess = true
            end)
            
            -- Format 2: Trade with items table
            if not tradeSuccess then
                pcall(function()
                    tradeRemote:InvokeServer({
            ...
            -- Format 3: Trade with player name
            if not tradeSuccess then
                pcall(function()
                    tradeRemote:InvokeServer(selectedTradePlayer, uuidList)
                    tradeSuccess = true
                end)
            end
    ...
    if success and not err then
        ...
        Rayfield:Notify({
            Title = "✅ Trade Success",
            Content = string.format("Traded %s to %s!", formatCurrency(totalValue), selectedTradePlayer),
            Duration = 5,
            Image = 4483362458
        })
    else
...
local function autoTradeLoop()
    Rayfield:Notify({
        ...
        Content = string.format("Target: %s | Amount: %s", selectedTradePlayer, formatCurrency(selectedTradeAmount)),
        Duration = 5,
        Image = 4483362458
    })
    
    while autoTradeEnabled do
        -- Check if reached 100 success
        if tradeSuccessCount >= 100 then
            autoTradeEnabled = false
            Rayfield:Notify({
                Title = "🎉 Trade Complete!",
                Content = string.format("Completed 100 trades! Total: %s", formatCurrency(totalCoinConverted)),
                Duration = 10,
                Image = 4483362458
            })
            break
        ...
        local currentValue = getInventoryValue()
        
        if currentValue >= selectedTradeAmount then
            -- Execute trade
            executeTrade()
            ...
            Rayfield:Notify({
                ...
                Content = string.format("Current: %s / Target: %s", 
    ...
    Rayfield:Notify({
        ...
        Content = string.format("Stats - Success: %d | Failed: %d", tradeSuccessCount, tradeFailedCount),
        Duration = 5,
        Image = 4483362458
    })
end
...
TradeTab:CreateSection("💰 Trade Amount Presets")

TradeTab:CreateLabel("Click to set trade amount target:")

local tradeAmounts = {
    {name = "💵 $100K", value = 100000, emoji = "💵"},
    {name = "💸 $500K", value = 500000, emoji = "💸"},
    {name = "💎 $1M", value = 1000000, emoji = "💎"},
    {name = "🏆 $2M", value = 2000000, emoji = "🏆"},
    {name = "👑 $5M", value = 5000000, emoji = "👑"}
}
...
TradeTab:CreateButton({
    Name = "💼 Check Current Inventory Value",
    Callback = function()
        local value = getInventoryValue()
        local fishList, _ = getFishToTrade(selectedTradeAmount)
        
        Rayfield:Notify({
            Title = "📊 Inventory Status",
            Content = string.format(
                "Total Value: %s\nTradeable Fish: %d items\nTarget: %s",
...
TradeTab:CreateToggle({
    Name = "🔄 Auto Trade (Until 100 Success)",
    CurrentValue = false,
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            if selectedTradeAmount <= 0 then
                Rayfield:Notify({
                    Title = "❌ Trade Error",
                    Content = "Please set trade amount first!",
                    Duration = 3
                })
                autoTradeEnabled = false
                return
            ...
            Rayfield:Notify({
                Title = "🛑 Auto Trade Stopped",
                Content = "Auto trade has been disabled",
                Duration = 3
            })
        end
    end,
...
TradeTab:CreateSection("📈 Trade Progress Monitor")
...
TradeTab:CreateLabel("⚠️ WARNING: Use at your own risk!")
TradeTab:CreateLabel("📌 Make sure target player is active")
TradeTab:CreateLabel("💡 Fish won't be traded if favorited")
TradeTab:CreateLabel("⏱️ Wait 3-5 seconds between trades")
TradeTab:CreateLabel("🎯 Trade stops at 100 successful trades")
