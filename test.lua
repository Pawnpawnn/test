...
-- Variabel untuk menyimpan button event
local eventButtons = {}
...
local function updateEventButtons()
    ...
    -- Hapus button event lama
    for _, button in pairs(eventButtons) do
        button:Destroy()
    end
    eventButtons = {}
    
    -- Buat button untuk setiap event yang aktif
    ...
    if #events == 0 then
        local noEventLabel = TeleportTab:CreateLabel("📭 No active events found")
        table.insert(eventButtons, noEventLabel)
    end
end

-- Button untuk refresh events
TeleportTab:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({
            ...
            Content = "Updated active events list",
            Duration = 2,
            Image = 4483362458
        })
    end,
...
-- watcher event (auto detect perubahan)
task.spawn(function()
    while task.wait(5) do -- scan tiap 5 detik
        local newEvents = ScanActiveEvents() or {}

        -- bandingkan dengan eventButtons lama
        if hasEventChanged(newEvents) then
            updateEventButtons()
        end
    end
end)
