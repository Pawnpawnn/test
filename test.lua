...
local function getFishToTrade(targetValue)
    ...
    local success = pcall(function()
        ...
        if type(inventoryData) == "table" then
            -- Filter hanya fish yang tidak difavoritkan dan memiliki value > 0
            local tradableFish = {}
            for _, item in ipairs(inventoryData) do
                if not item.Favorited and item.Value and item.Value > 0 then
                    table.insert(tradableFish, {
                        ...
                        Value = item.Value,
                        Id = item.Id,
                        Name = item.Name or "Unknown Fish"
                    })
                end
            end
            
            -- Sort by value (lowest first untuk efisiensi)
            table.sort(tradableFish, function(a, b)
                return (a.Value or 0) < (b.Value or 0)
            end)
            
            -- Pilih fish sampai mencapai target value
            for _, fish in ipairs(tradableFish) do
                if currentValue < targetValue then
                    table.insert(fishList, fish)
                    currentValue += fish.Value
                    
                    -- Stop jika sudah mencapai atau melebihi target
                    if currentValue >= targetValue then
                        break
                    end
                else
...
local function executeTrade()
    ...
    local success, result = pcall(function()
        ...
        if #uuidList == 0 then
            tradeError = "No valid fish UUIDs found"
            return false
        end
        ...
        local methods = {
            ...
            function() return tradeRemote:FireServer(targetPlayer.UserId, uuidList) end
        }
