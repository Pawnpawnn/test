...
local function findLuckyServer()
    Rayfield:Notify({
        Title = "🔍 Mencari Server Luck",
        Content = "Target: Server dengan Luck x4 atau x8...",
        Duration = 4,
        Image = 4483362458
    })
    
    local maxAttempts = 20
    local targetMultiplier = 4  -- Minimal x4
    
    for attempt = 1, maxAttempts do
        -- Tunggu game load sepenuhnya (8 detik cukup)
        task.wait(8)
        
        -- Scan UI untuk Server Luck
        local currentLuck, luckText = scanServerLuck()
        
        if currentLuck then
            if currentLuck >= targetMultiplier then
                -- 🎉 KETEMU SERVER BAGUS!
                Rayfield:Notify({
                    Title = "🎉 Server Luck Ditemukan!",
                    Content = string.format("Server Luck: x%d | Tinggal di server ini!", currentLuck),
                    Duration = 8,
                    Image = 4483362458
                })
                return currentLuck, true
            else
                -- Server cuma x2, lanjut hop
                Rayfield:Notify({
                    Title = "➡️ Server x2 Only",
                    Content = string.format("Luck hanya x%d. Lanjut mencari x4/x8... (%d/%d)", 
                        currentLuck, attempt, maxAttempts),
                    Duration = 3,
                    Image = 4483362458
                })
            end
        else
            -- Ga ketemu text Server Luck sama sekali
            Rayfield:Notify({
                Title = "🔄 No Luck Found",
                Content = string.format("Tidak ada Server Luck. Hopping... (%d/%d)", attempt, maxAttempts),
                Duration = 2,
                Image = 4483362458
            })
        end
        
        -- Jika belum ketemu x4/x8, hop ke server lain
        if attempt < maxAttempts and (not currentLuck or currentLuck < targetMultiplier) then
            ServerHop()
        end
    end
    
    -- Jika setelah max attempts belum ketemu x4/x8
    Rayfield:Notify({
        Title = "❌ Pencarian Selesai",
        Content = "Tidak menemukan server x4/x8 setelah " .. maxAttempts .. " percobaan",
        Duration = 6
    })
    return nil, false
end

local function findMaxLuckServer()
    Rayfield:Notify({
        Title = "🎯 Target: MAX LUCK",
        Content = "Mencari Server Luck x8...",
        Duration = 4,
        Image = 4483362458
    })
    ...
    for attempt = 1, maxAttempts do
        ...
        if currentLuck == 8 then
            Rayfield:Notify({
                Title = "🎉 MAX LUCK x8 DITEMUKAN!",
                Content = "Server Luck x8! Auto stop di server ini!",
                Duration = 10,
                Image = 4483362458
            })
            return 8
        elseif currentLuck then
            Rayfield:Notify({
                Title = "➡️ Bukan x8",
                Content = string.format("Luck: x%d. Lanjut mencari x8... (%d/%d)", 
                    currentLuck, attempt, maxAttempts),
                Duration = 2
            })
        else
            Rayfield:Notify({
                Title = "🔄 No Luck",
                Content = string.format("Tidak ada luck. Hopping... (%d/%d)", attempt, maxAttempts),
                Duration = 2
            })
        end
    ...
    Rayfield:Notify({
        Title = "❌ x8 Not Found",
        Content = "Tidak ketemu x8 setelah " .. maxAttempts .. " attempts",
        Duration = 6
    })
end

local function checkCurrentServerLuck()
    local luck, text = scanServerLuck()
    if luck then
        Rayfield:Notify({
            Title = "📊 Server Luck Status",
            Content = string.format("Server ini: %s", text),
            Duration = 5,
            Image = 4483362458
        })
    else
        Rayfield:Notify({
            ...
            Content = "Tidak ada Server Luck di server ini",
            Duration = 3
        })
    end
end
...
local ServerSection = MiscTab:CreateSection("🍀 Server Luck System")

-- Button utama cari server luck
MiscTab:CreateButton({
    Name = "🎯 Cari Server Luck x4/x8",
    Callback = function()
        findLuckyServer()
    end,
})

-- Button cari MAX luck x8
MiscTab:CreateButton({
    Name = "🎉 Cari MAX Luck x8",
    Callback = function()
        findMaxLuckServer()
    end,
})

-- Button cek luck server saat ini  
MiscTab:CreateButton({
    Name = "📊 Cek Server Luck Sekarang",
    Callback = function()
        checkCurrentServerLuck()
    end,
})
...
local ServerHopSection = MiscTab:CreateSection("Server Management")
...
Rayfield:Notify({
    Title = "Script Loaded!",
    Content = "Fish It Auto V2.5 + Server Luck System loaded successfully!",
    Duration = 5,
    Image = 4483362458
})
