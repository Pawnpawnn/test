...
local function autoFishingV5Loop()
    while autoFishingV5Enabled do
        local ok, err = pcall(function()
            ...
            -- Jalankan semua phase kecuali finish
            if equipRemote then equipRemote:FireServer(1) end
            if rodRemote then rodRemote:InvokeServer(tick()) end
            if miniGameRemote then 
               miniGameRemote:InvokeServer(math.random(-150,150)/100, math.random(80,120)/100)
               
               -- Finish dengan delay tapi loop tetap jalan
               delay(2.1, function()
                   if finishRemote and autoFishingV5Enabled then
        ...
        if not ok then warn("[V5 ERROR]: " .. tostring(err)) end
        task.wait(0.01)
    end
