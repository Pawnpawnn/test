...
PlayerDropdown = MainTab:CreateDropdown({
    ...
    Callback = function(selected)
        -- FIX: Handle "No players online" case
        if selected == "No players online" then
            NotifyError("No Players", "No other players are online!")
            return
        end
        
        -- pake 'selected' bukan 'username'
        ...
        if targetPlayer then
            ...
            NotifyError("Player Not Found", "Could not find player: " .. selected)
        end
    end
})
