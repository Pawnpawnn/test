...
local statusLabel = create("TextLabel", {
    ...
    BackgroundTransparency = 1,
    Text = "🔴 Status: Idle\nScript: V.3.0\nNote: With Animation + Perfect System",
    Font = Enum.Font.GothamBold,
    TextSize = 10,
...
-- PERFECT CAST TOGGLE SECTION
local perfectSection = create("Frame", {
    Parent = contentFrame,
    Size = UDim2.new(1, 0, 0, 40),
...
create("UICorner", {Parent = perfectSection, CornerRadius = UDim.new(0, 7)})
create("UIStroke", {Parent = perfectSection, Color = Color3.fromRGB(40, 60, 90), Thickness = 1})

local perfectTitle = create("TextLabel", {
    Parent = perfectSection,
    Size = UDim2.new(0.55, 0, 1, 0),
    Position = UDim2.new(0, 9, 0, 0),
    BackgroundTransparency = 1,
    Text = "⭐ Perfect Cast System",
    Font = Enum.Font.GothamBold,
    TextSize = 9,
...
local perfectToggle = create("TextButton", {
    Parent = perfectSection,
    Size = UDim2.new(0, 72, 0, 27),
    Position = UDim2.new(1, -78, 0, 6),
...
create("UICorner", {Parent = perfectToggle, CornerRadius = UDim.new(0, 6)})
...
addHover(perfectToggle, Color3.fromRGB(50, 150, 50), Color3.fromRGB(70, 170, 70))
...
local function togglePerfectCast()
    perfectCastEnabled = not perfectCastEnabled
    
    if perfectCastEnabled then
        perfectToggle.Text = "ENABLED"
        perfectToggle.BackgroundColor3 = Color3.fromRGB(50, 150, 50)
        updateStatus("⭐ Perfect Cast: ENABLED", Color3.fromRGB(100, 255, 100))
    else
        perfectToggle.Text = "DISABLED"
        perfectToggle.BackgroundColor3 = Color3.fromRGB(180, 60, 60)
        updateStatus("❌ Perfect Cast: DISABLED", Color3.fromRGB(255, 100, 100))
    end
end
...
local function autoFishingV3Loop()
    ...
    while autoFishingV3Enabled do
        local ok, err = pcall(function()
            ...
            -- PHASE 3: PERFECT CAST COORDINATES
            local baseX, baseY = -0.7499996, 1
            local x, y
            
            if perfectCastEnabled then
                -- Perfect cast coordinates dengan variasi micro
                x = baseX + (math.random(-100, 100) / 10000000)
                y = baseY + (math.random(-100, 100) / 10000000)
                updateStatus("⭐ Perfect Cast Activated!", Color3.fromRGB(100, 255, 100))
            else
                -- Random coordinates (non-perfect)
                x = math.random(-1000, 1000) / 1000
                y = math.random(0, 1000) / 1000
                updateStatus("🎯 Random Cast", Color3.fromRGB(255, 200, 100))
            end
            ...
            -- Always perfect finish ketika perfect cast enabled
            local willSucceed = perfectCastEnabled and true or (math.random(1, 100) <= 75)
            ...
            if willSucceed then
                ...
                updateStatus("❌ Missed Catch", Color3.fromRGB(255, 100, 100))
            end
...
-- Perfect Cast Toggle
perfectToggle.MouseButton1Click:Connect(togglePerfectCast)
...
updateStatus("✅ Script Loaded Successfully\nWith Animation System", Color3.fromRGB(100, 255, 100))
