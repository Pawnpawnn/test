...
local rodEquipped = false  -- Track rod status
...
-- **FIXED: Fungsi Auto Fishing V5 - OPTIMAL**
local function autoFishingV5Loop()
    while autoFishingV5Enabled do
        local ok, err = pcall(function()
            fishingV5Active = true
            
            -- **PHASE 1: EQUIP fishing rod (HANYA SEKALI SAJA)**
            if equipRemote and not rodEquipped then
                equipRemote:FireServer(1)
                rodEquipped = true
                task.wait(0.1)  -- Kasih waktu buat equip
            end

            -- **PHASE 2: CHARGE fishing rod (OPTIMAL SPAM)**
            if rodRemote then
                local timestamp = tick()
                for i = 1, 10 do  -- Reduced spam, cukup 3x
                    rodRemote:InvokeServer(timestamp + i)
                    task.wait(0.0001)
                end
            end

            -- **PHASE 3: START Mini Game (OPTIMAL)**
            if miniGameRemote then
                local baseX, baseY = -0.7499996, 1
                for i = 1, 5 do  -- Reduced to 5x spam
                    local x = baseX + (math.random(-500, 500) / 10000000)
                    local y = baseY + (math.random(-500, 500) / 10000000)
                    miniGameRemote:InvokeServer(x, y)
                    task.wait(0.0001)
                end
                task.wait(1.5)  -- Wait for minigame
            end
            
            -- **PHASE 4: FINISH Fishing (TARGETED SPAM)**
            if finishRemote then
                for i = 1, 15 do  -- Optimal spam finish
        ...
        task.wait(0.1)  -- Optimal main loop delay
    end
    fishingV5Active = false
end

-- **IMPROVED Exclaim Detection - EFFICIENT**
task.spawn(function()
    ...
    if success and exclaimEvent then
        exclaimEvent.OnClientEvent:Connect(function(data)
            if autoFishingV5Enabled and data and data.TextData
                ...
                if head and data.Container == head then
                    print("🎣 V5 - Exclaim detected! Efficient recasting...")
                    task.spawn(function()
                        -- **EFFICIENT spam ketika dapat fish**
                        for i = 1, 15 do  -- Reduced to 15x (masih efektif)
                            task.wait(0.000001)
                            pcall(function()
                                if finishRemote then 
                                    finishRemote:FireServer(true)
        ...
        print("✅ V5 Exclaim detection activated - EFFICIENT MODE")
    end
end

-- Start Fishing V5 - OPTIMAL MODE
local function StartFishingV5()
    ...
    print("🎣 FISHING V5 STARTED - OPTIMAL MODE")
    autoFishingV5Enabled = true
    rodEquipped = false  -- Reset rod status
    ...
    if Window then
        Rayfield:Notify({
            ...
            Content = "Optimal Efficiency Mode Activated!",
            Duration = 5,
            Image = 4483362458
        })
    end
