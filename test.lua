...
local IslandButton = TeleportTab:CreateButton({
    Name = "🏝️ Teleport to Selected Island",
    Callback = function()
        local selectedIsland = "Kohana" -- Default
        
        -- Get selected island from saved config
        pcall(function()
            selectedIsland = Rayfield.Flags.IslandDropdown.CurrentOption or "Kohana"
        end)
...
for islandName, coords in pairs(islandCoords) do
        ...
        local NPCDropdown = TeleportTab:CreateDropdown({
            ...
            Callback = function(Option)
                ...
                if npc and npc:IsA("Model") then
                    ...
                    if hrp then
                        local success, err = teleportToPosition(hrp.Position + Vector3.new(0, 3, 0))
                        ...
                        Rayfield:Notify({
                            Title = "Error",
                            Content = "NPC HumanoidRootPart not found",
                            Duration = 3
                        })
                    end
                else
