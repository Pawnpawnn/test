...
local customDelayV1 = 1
local BypassDelayV1 = 0.5
local delayInitializedV1 = false

local function getValidRodNameV1()
    local player = Players.LocalPlayer
...
local function updateDelayBasedOnRodV1(showNotify)
    if delayInitializedV1 then return end
    local rodName = getValidRodNameV1()
    if rodName and RodDelaysV1[rodName] then
        customDelayV1 = RodDelaysV1[rodName].custom
        BypassDelayV1 = RodDelaysV1[rodName].bypass
        delayInitializedV1 = true
        if showNotify and autoFishingEnabled then
            Rayfield:Notify({
                Title = "Rod Detected",
                Content = string.format("Detected Rod: %s | Delay: %.2fs | Bypass: %.2fs", rodName, customDelayV1, BypassDelayV1),
                Duration = 5,
                Image = 4483362458
            })
        end
        ...
        if showNotify and autoFishingEnabled then
            Rayfield:Notify({
                Title = "Rod Detection Failed",
                Content = "No valid rod found. Default delay applied.",
                Duration = 5
            })
        end
    end
...
local function setupRodWatcherV1()
    local player = Players.LocalPlayer
    local display = player.PlayerGui:WaitForChild("Backpack"):WaitForChild("Display")
    display.ChildAdded:Connect(function()
        task.wait(0.05)
        if not delayInitializedV1 and autoFishingEnabled then
            updateDelayBasedOnRodV1(true)
        end
    end)
end
setupRodWatcherV1()
...
local function autoFishingLoop()
    ...
    fishingActive = false
    delayInitializedV1 = false -- Reset untuk next activation
end
...
local FishingV1Toggle = MainTab:CreateToggle({
    Name = "🎣 Auto Fishing V1 (Perfect + Delay + Rod Detection)",
    CurrentValue = false,
    ...
    Callback = function(Value)
        ...
        if Value then
            Rayfield:Notify({
                ...
                Content = "Auto Fishing V1 Started! Rod detection active...",
                Duration = 3,
                Image = 4483362458
            })
...
local InfoLabel = MiscTab:CreateLabel("🐟 Fish It Premium V2.5")
local InfoLabel2 = MiscTab:CreateLabel("Made by: Codepikk")
local InfoLabel3 = MiscTab:CreateLabel("Discord: codepikk")
local InfoLabel4 = MiscTab:CreateLabel("Donate if you're happy! :)")
...
Rayfield:Notify({
    Title = "Script Loaded!",
    Content = "Fish It Auto V2.5 loaded successfully! Rod detection for V1 only.",
    Duration = 5,
    Image = 4483362458
})
