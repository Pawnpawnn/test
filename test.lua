...
-- ⚡ HOOK THE FISHING SYSTEM
local originalFireServer
local hookActive = false

local function enableFishingHook()
    ...
    originalFireServer = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        -- Jika ini fishing completion remote DAN fishing enabled
        if InstantFishing.Enabled and method == "FireServer" then
            if self == finishRemote then
                -- 🚀 INSTANT COMPLETION - Auto finish tanpa menunggu exclaim!
                local currentTime = tick()
                
                -- Jika baru mulai cast, tunggu sebentar lalu auto complete
                if currentTime - InstantFishing.LastCastTime > 1.5 then
                    InstantFishing.CastCount = InstantFishing.CastCount + 1
                    
                    -- Auto complete setelah delay sangat singkat
                    task.spawn(function()
                        -- Tunggu waktu random antara 0.1-0.3 detik lalu auto success
                        local waitTime = math.random(10, 30) / 100 * InstantFishing.SpeedMultiplier
                        task.wait(waitTime)
                        
                        -- Fire multiple success completions
                        for i = 1, 5 do
                            pcall(function()
                                originalFireServer(self, true) -- Force success
                            end)
                            if i < 3 then
                                task.wait(0.001)
                            end
                    ...
                    InstantFishing.LastCastTime = currentTime
    ...
    hookActive = true
    print("🎣 FISHING HOOK ENABLED - ULTRA INSTANT MODE")
end

local function disableFishingHook()
    if hookActive and originalFireServer then
        hookmetamethod(game, "__namecall", originalFireServer)
        hookActive = false
        print("🎣 Fishing hook disabled")
    end
end
...
local function updateRodModifiers()
        ...
        for attrName, attrValue in pairs(equippedRod:GetAttributes()) do
            ...
            if nameLower:find("reel") and nameLower:find("faster") then
                local speedBonus = tonumber(string.match(attrValue, "%d+")) or 
                                 tonumber(string.match(attrName, "%d+")) or 0
                if speedBonus > 0 then
                    speedMultiplier = math.max(0.2, 1.0 - (speedBonus / 100)) -- Super aggressive
                    print("🎯 Rod Modifier: " .. speedBonus .. "% faster -> Multiplier: " .. speedMultiplier)
                end
            end
        end
    ...
    InstantFishing.SpeedMultiplier = speedMultiplier
    return speedMultiplier
end

-- 🚀 ULTRA FAST FISHING LOOP - NO WAITING FOR EXCLAIM
local function ultraFishingLoop()
    ...
    while InstantFishing.Enabled do
        local success, err = pcall(function()
            ...
            task.wait(0.15 * InstantFishing.SpeedMultiplier)
            
            -- Start cast
            local timestamp = workspace:GetServerTimeNow()
            rodRemote:InvokeServer(timestamp)
            task.wait(0.1 * InstantFishing.SpeedMultiplier)
            
            -- Perfect cast
            local x, y
            if InstantFishing.PerfectCast then
                x = -0.7499996 + (math.random(-5, 5) / 10000000)
                y = 1 + (math.random(-5, 5) / 10000000)
            else
            ...
            -- 🚀 INSTANT COMPLETION TRIGGER
            InstantFishing.LastCastTime = tick()
            
            -- Fire completion immediately dengan multiple calls
            task.spawn(function()
                local completionDelay = math.random(8, 15) / 100 * InstantFishing.SpeedMultiplier
                task.wait(completionDelay)
                
                -- Multiple completion calls untuk memastikan success
                for i = 1, 8 do
                    pcall(function()
                        finishRemote:FireServer(true)
                    end)
                    if i <= 3 then
                        task.wait(0.0005) -- Ultra fast untuk 3 pertama
                    else
                        task.wait(0.001)
                    end
                end
            ...
            -- Dynamic cooldown berdasarkan sequence
            local baseCooldown = math.random(3, 8) / 100 * InstantFishing.SpeedMultiplier
            
            -- Adjust berdasarkan cast sequence
            if castSequence % 5 == 0 then
                baseCooldown = baseCooldown * 1.5 -- Slow down setiap 5 casts
            elseif castSequence % 10 == 0 then
                baseCooldown = baseCooldown * 2.0 -- Slow down setiap 10 casts
            end
            
            task.wait(math.max(0.02, baseCooldown))
            
        end)
        
        if not success then
            warn("Ultra Fishing Error: " .. tostring(err))
            task.wait(0.3)
        end
    end
end
...
local function monitorFishThreshold()
    task.spawn(function()
        while InstantFishing.Enabled do
            ...
            task.wait(0.2) -- Check lebih sering
        end
    end)
end

-- Start Ultra Fishing
local function startUltraFishing()
    ...
    Rayfield:Notify({
        Title = "🚀 ULTRA FISHING Started",
        Content = "NO EXCLAIM WAITING! Instant completion!",
        Duration = 4,
        Image = 4483362458
    })
    
    print("🚀 ULTRA INSTANT FISHING ACTIVATED!")
    print("⚡ HOOK METHOD: Intercept fishing completion")
    print("🎯 AUTO SUCCESS: No waiting for exclaim")
    print("💨 SPEED: " .. (InstantFishing.SpeedMultiplier * 100) .. "%")
end

local function stopUltraFishing()
    InstantFishing.Enabled = false
    disableFishingHook()
    
    Rayfield:Notify({
        Title = "🛑 Ultra Fishing Stopped",
        Content = "Instant fishing system disabled",
        Duration = 3
    })
end
...
-- Main Tab
local MainTab = Window:CreateTab("🎣 ULTRA Fishing", 4483362458)

MainTab:CreateToggle({
    Name = "🚀 ULTRA INSTANT FISHING",
    CurrentValue = false,
    Flag = "UltraFishingToggle",
...
MainTab:CreateToggle({
    ...
    Callback = function(Value)
        InstantFishing.PerfectCast = Value
        Rayfield:Notify({
            Title = "Perfect Cast",
...
MainTab:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({
            Title = "Rod Scan",
            Content = "Speed Multiplier: " .. (InstantFishing.SpeedMultiplier * 100) .. "%",
            Duration = 4,
            Image = 4483362458
        })
    end,
