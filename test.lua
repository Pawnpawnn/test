Edit ukuran frame menjadi 320 x beraa gitu pokonya bikin kotak square. karena bisa di scroll 
                ...
                if type(items) == "table" then
                    ...
                    for _, item in ipairs(items) do
                        ...
                        if base and base.Data and allowedTiers[base.Data.Tier] and not item.Favorited then
                            ...
                            updateStatus("⭐ Favorited: " .. base.Name .. " [" .. base.Data.Tier .. "]", Color3.fromRGB(255, 215, 0))
                            task.wait(0.3)
                        end
                    end
                    
                    if totalFavorited > 0 then
                        updateStatus("✅ Favorited " .. totalFavorited .. " items!", Color3.fromRGB(100, 255, 100))
                    else
                        updateStatus("ℹ️ No new rare items", Color3.fromRGB(255, 255, 100))
                    end
                end
            end)
            
            if not success then
                updateStatus("❌ Auto Favorite Error", Color3.fromRGB(255, 100, 100))
            end
...
local function setupRemotes()
    ...
    favoriteRemote = net:WaitForChild("RE/FavoriteItem")  -- INI YANG DITAMBAH
end
...
-- Main Frame - DIUBAH MENJADI 320x380 (SQUARE/KOTAK)
local mainFrame = create("Frame", {
    Name = "MainFrame",
    Parent = screenGui,
    Size = UDim2.new(0, 320, 0, 380), -- DIUBAH: Lebar 320, Tinggi 380
    Position = UDim2.new(0.5, -160, 0.5, -190), -- DIUBAH: Posisi disesuaikan
...
-- Content Frame untuk menampung semua section - DIUBAH MENJADI SCROLLING FRAME
local contentFrame = create("ScrollingFrame", {
    ...
    CanvasSize = UDim2.new(0, 0, 0, 500) -- DIUBAH: Tinggi canvas diperbesar untuk scroll
})
...
-- Main Tab Content
local mainTab = create("Frame", {
    Name = "MainTab",
    Parent = contentFrame,
    Size = UDim2.new(1, 0, 0, 500), -- DIUBAH: Tinggi diperbesar
    BackgroundTransparency = 1,
    BorderSizePixel = 0,
    Visible = true
})
...
local function switchTab(tabName)
    ...
    if tabName == "Main" then
        ...
        contentFrame.CanvasSize = UDim2.new(0, 0, 0, 500) -- Sesuaikan ukuran scroll
...
favoriteBtn.MouseButton1Click:Connect(function()
    ...
    if autoFavoriteEnabled then
        ...
        updateStatus("🟢 Status: Auto Favorite Started", Color3.fromRGB(100, 255, 100))
        startAutoFavorite()
    else
        favoriteBtn.Text = "START"
        favoriteBtn.BackgroundColor3 = Color3.fromRGB(180, 80, 180)
        updateStatus("🔴 Status: Auto Favorite: Disabled")
    end
end)
