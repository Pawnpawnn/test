...
-- Fishing V4 (Ultra Instant Bite) - COMPLETELY REWORKED
local function ExecuteUltraBiteCycle()
    ...
    pcall(function()
        ...
        if equipRemote then 
            ...
            task.wait(0.3) -- Increased wait for equipment
        end
        ...
        if ChargeRod then 
            ...
            task.wait(0.3) -- Increased wait for casting
        end
        
        -- Step 3: ULTRA INSTANT CATCH - No waiting for bite detection
        if debugMode then print("[DEBUG] 🚀 ULTRA INSTANT CATCH - No wait!") end
        
        -- Method 1: Direct minigame completion
        if StartMini and FinishFish then
            pcall(function()
                -- Try multiple parameter combinations for minigame
                ...
                if not success then
                    ...
                    pcall(function()
                        StartMini:InvokeServer(-1.5, 0.9)
                    end)
                end
                
                if debugMode then print("[DEBUG] Minigame attempted") end
                
                task.wait(0.15) -- Short wait for minigame processing
        ...
        -- Method 2: Direct FishCaught (immediate)
        if FishCaught and catches == 0 then
            pcall(function()
                FishCaught:FireServer()
                catches = catches + 1
                if debugMode then print("[DEBUG] Direct catch method successful") end
            end)
        end
        
        -- Method 3: Alternative fishing completion
        if catches == 0 then
            pcall(function()
                -- Try any fishing-related remote
                for _, remote in pairs({StopFishing, FishCaught}) do
                    if remote then
                        pcall(function()
                            remote:FireServer()
                            catches = catches + 1
                            if debugMode then print("[DEBUG] Alternative catch successful") end
                        end)
                        break
                    end
                end
        ...
        -- Step 4: Force stop fishing to reset
        if StopFishing then
            pcall(function()
