...
local function testSignatures(remote)
    local tries = {
        function() return remote.FireServer(remote, true) end,
        function() return remote.FireServer(remote, true, os.clock()) end,
        function() return remote.FireServer(remote, os.clock()) end,
        function() return remote.FireServer(remote, {Timestamp = os.clock(), Result = true}) end,
        function() return remote.FireServer(remote) end
    }

    for i,fn in ipairs(tries) do
        local ok, res = pcall(fn)
        print(("[TEST] try %d => ok=%s, result=%s"):format(i, tostring(ok), tostring(res)))
        task.wait(0.05)
    end
end

-- panggil ini hanya kalau kamu paham risikonya
-- testSignatures(finishRemote)
