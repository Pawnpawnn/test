...
-- Auto Fishing V5 Loop yang Fixed
local function autoFishingV5Loop()
    while autoFishingV5Enabled do
        if not isFishing then  -- Hanya jalankan jika tidak sedang proses fishing
            pcall(function()
                isFishing = true
                fishingV5Active = true
                
                print("🎣 Memulai fishing cycle...")
                ...
                print("🎣 Umpan dilempar, menunggu ikan...")
                
                -- Tunggu maksimal 10 detik untuk ikan strike, lalu lanjut cycle berikutnya
                local waitTime = 0
                while isFishing and waitTime < 10 do
                    task.wait(0.5)
                    waitTime = waitTime + 0.5
                end
                
                -- Jika masih fishing setelah 10 detik, force finish
                if isFishing then
                    print("⏰ Timeout, melanjutkan cycle berikutnya...")
                    isFishing = false
                end
...
-- Exclaim Detection yang BENERAN narik
task.spawn(function()
    ...
    if success and exclaimEvent then
            ...
            pcall(function()
                if autoFishingV5Enabled and data and data.TextData
                    ...
                    if head and data.Container == head then
                        print("⚡ TANDA SERU! Menarik ikan...")
                        
                        -- STOP dulu cycle yang sedang berjalan
                        isFishing = false
                        task.wait(0.3)  -- Kasih delay dikit biar game kebaca
        ...
        print("✅ Exclaim detection activated - BENERAN narik!")
    else
        warn("❌ Exclaim detection failed")
    end
end)
...
local function StartFishingV5()
    ...
    pcall(function()
        print("🎣 FISHING V5 STARTED - Fixed Version")
        ...
        if Window then
            Rayfield:Notify({
                ...
                Content = "Fixed Version - Bisa narik!",
                Duration = 3,
                Image = 4483362458
            })
        end
