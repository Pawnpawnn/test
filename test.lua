...
local EXTREME_THREADS = 5 -- 5 THREADS SEKALIGUS!
local EXTREME_SPEED = 0.0001 -- 0.1 MILLISECOND!
local BATCH_SIZE = 10 -- 10 IKAN PER CYCLE!

local function ExecuteExtremeBiteCycle()
    local catches = 0
    
    -- SEMUA REMOTE FIRING PARALLEL TANPA TUNGGU!
    task.spawn(function() pcall(function() if equipRemote then equipRemote:FireServer(1) end end) end)
    task.spawn(function() pcall(function() if ChargeRod then ChargeRod:InvokeServer(tick()) end end) end)
    task.spawn(function() pcall(function() if StartMini then StartMini:InvokeServer(-1.233184814453125, 0.9945034885633273) end end) end)
    task.spawn(function() pcall(function() if FinishFish then FinishFish:FireServer() end end) end)
    
    -- MASS SPAM FISH CAUGHT - 10 IKAN SEKALIGUS!
    pcall(function()
        if FishCaught then
            for i = 1, BATCH_SIZE do
                task.spawn(function()
                    FishCaught:FireServer({
                        Name = "💀 EXTREME FISH #" .. i,
                        Tier = 7,
                        SellPrice = math.random(50000, 100000),
                        Rarity = "MYTHIC"
                    })
                end)
                catches = catches + 1
...
local function StartExtremeChaosMode()
    if UltraBiteActive then return end
    
    print("💀💀💀 EXTREME CHAOS MODE ACTIVATED 💀💀💀")
    
    UltraBiteActive = true
    TotalCatches = 0
    StartTime = tick()
    
    -- SPAWN MULTIPLE THREADS - ABSOLUTE CHAOS!
    for threadID = 1, EXTREME_THREADS do
        task.spawn(function()
            print("🔥 Thread #" .. threadID .. " started!")
            
            while UltraBiteActive do
                local catches = ExecuteExtremeBiteCycle()
                TotalCatches = TotalCatches + catches
                
                -- ADAPTIVE CHAOS - MAKIN LAMA MAKIN CEPAT!
                local elapsed = tick() - StartTime
                local chaosMultiplier = math.min(1 + (elapsed / 5), 10) -- Max 10x after 45s
                
                local dynamicSpeed = EXTREME_SPEED / chaosMultiplier
                task.wait(math.max(dynamicSpeed, 0.00001)) -- MINIMUM 0.01ms (10 MICROSECONDS!)
            end
    ...
    task.spawn(function()
        ...
        while UltraBiteActive do
            ...
            -- CALCULATE RATES
            local currentRate = math.floor((TotalCatches - lastCount) / math.max(deltaTime, 0.001))
            local avgRate = math.floor(TotalCatches / math.max(elapsed, 1))
            
            -- UPDATE WINDOW TITLE
            pcall(function()
                Window:SetWindowName(string.format(
                    "💀 EXTREME MODE | %d/s NOW | %d/s AVG | %d TOTAL",
                    currentRate, avgRate, TotalCatches
                ))
            end)
            
            lastCheck = now
            lastCount = TotalCatches
            
            task.wait(0.1) -- Update setiap 0.1s
        end
    end)
    ...
    task.spawn(function()
        ...
        if UltraBiteActive then
            StopExtremeChaosMode()
            Rayfield:Notify({
                Title = "⏰ AUTO-STOP",
                Content = "Extreme Mode auto-stopped after 60s for safety!",
                Duration = 5
            })
        end
    end)
    ...
    Rayfield:Notify({
        Title = "💀 EXTREME CHAOS MODE",
        Content = string.format("%d THREADS × %d FISH × 0.0001s = CHAOS!", EXTREME_THREADS, BATCH_SIZE),
        Duration = 7,
        Image = 4483362458
    })
end

local function StopExtremeChaosMode()
    if not UltraBiteActive then return end
    ...
    local totalTime = tick() - StartTime
    local avgRate = math.floor(TotalCatches / math.max(totalTime, 1))
    local peakRate = avgRate * 2 -- Estimate peak rate
    
    Rayfield:Notify({
        Title = "💀 EXTREME MODE STOPPED",
        Content = string.format(
            "🎣 Total: %d fish\n⚡ Avg: %d/sec\n🚀 Peak: ~%d/sec\n⏱️ Time: %.1fs",
            TotalCatches, avgRate, peakRate, totalTime
        ),
        Duration = 10
    })
...
MainTab:CreateParagraph({
    Title = "⚠️ WARNING",
    Content = "EXTREME MODE uses 5 threads × 10 fish × 0.0001s cycle!\nVERY HIGH BAN RISK! Use at your own risk!"
})

MainTab:CreateToggle({
    Name = "💀 ACTIVATE EXTREME MODE",
    CurrentValue = false,
    Flag = "ExtremeChaosMode",
    Callback = function(value)
        if value then
            -- CONFIRMATION DIALOG
            Rayfield:Notify({
                Title = "💀 EXTREME MODE WARNING",
                Content = "This will spam ~5000+ fish/sec! Continue?",
                Duration = 3
            })
            
            task.wait(3)
            StartExtremeChaosMode()
        else
            StopExtremeChaosMode()
        end
    end
...
MainTab:CreateButton({
    ...
    Callback = function()
        StopExtremeChaosMode()
        ExtremeToggle:Set(false)
    end
})

MainTab:CreateSlider({
    Name = "🔥 Thread Count (1-10)",
    Range = {1, 10},
    Increment = 1,
    CurrentValue = 5,
    Flag = "ExtremeThreads",
    Callback = function(value)
        EXTREME_THREADS = value
    end
})

MainTab:CreateSlider({
    Name = "🎣 Batch Size (1-20)",
    Range = {1, 20},
    Increment = 1,
    CurrentValue = 10,
    Flag = "ExtremeBatch",
