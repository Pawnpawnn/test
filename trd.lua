...
local function scanPlayerInventory()
    ...
    local success, err = pcall(function()
        ...
        print("✅ Inventory scanned: " .. scannedCount .. " fish found")
    end)
...
PlayerDropdown = MainTab:CreateDropdown({
    ...
    Callback = function(selected)
        -- normalisasikan ke string dulu
        local selectedName = type(selected) == "table"
            and (selected[1] or selected.Selected or selected.Name)
            or selected

        if not selectedName then
            NotifyError("Player Error", "Invalid selection returned from dropdown")
            return
        end

        -- FIX: No players online
        if selectedName == "No players online" then
            NotifyError("No Players", "No other players are online!")
            return
        end
        ...
        if targetPlayer then
            ...
            NotifyError("Player Not Found", "Could not find player: " .. selectedName)
        end
    end
})
