...
local rodRemote, miniGameRemote, finishEvent, replicateText

-- Setup V5 Remotes (safe pcall)
local function SetupV5Remotes()
    local ok, err = pcall(function()
        local pkg = net -- assumed defined earlier in your script
        rodRemote = pkg:WaitForChild("RF/ChargeFishingRod")
        miniGameRemote = pkg:WaitForChild("RF/RequestFishingMinigameStarted")
        finishEvent = pkg:WaitForChild("RE/FishingCompleted")         -- RemoteEvent only (no Invoke)
        replicateText = pkg:WaitForChild("RE/ReplicateTextEffect")   -- Exclaim notifier
    end)
    return ok
end

-- small helper safe-calls
local function safeInvoke(remote, ...)
    if not remote then return false end
    local ok, res = pcall(function() return remote:InvokeServer(...) end)
    if not ok then warn("[safeInvoke] failed:", tostring(res)) end
    return ok, res
end
...
local function safeFire(remote, ...)
    ...
    if not ok then warn("[safeFire] failed:", tostring(res)) end
    return ok, res
end

-- Core V5 loop: equip -> charge -> request (NO finish here)
local function autoFishingV5Loop()
    while autoFishingV5Enabled do
        local ok, err = pcall(function()
            ...
            -- PHASE 2: CHARGE fishing rod (use server time)
            if rodRemote then
                local ts
                pcall(function() ts = workspace:GetServerTimeNow() end)
                if not ts then ts = tick() end
                safeInvoke(rodRemote, ts)
                task.wait(0.02)
            end

            -- PHASE 3: START Mini Game (provide small randomness near base coords)
            if miniGameRemote then
                local baseX, baseY = -0.7499996, 1
                local x = baseX + (math.random(-500, 500) / 10000000)
                local y = baseY + (math.random(-500, 500) / 10000000)
                -- pass server time again as 3rd arg (best practice)
                local ts2
                pcall(function() ts2 = workspace:GetServerTimeNow() end)
                if not ts2 then ts2 = tick() end
                safeInvoke(miniGameRemote, x, y, ts2)
                -- tiny wait to yield, but short so loop remains fast
                task.wait(0.05)
            end
        ...
        -- small loop pacing (will be accelerated by exclaim handler recast)
        task.wait(0.12)
    end
...
-- ===================================
-- When exclaim appears, spam finishEvent (RE/FishingCompleted) fast, then recast
-- This reproduces the timing-exploit method (used because there's no RF finish in dump)

task.spawn(function()
    -- ensure replicateText is present (if SetupV5Remotes not run yet, try to fetch now)
    if not replicateText then
        pcall(function()
            replicateText = net:WaitForChild("RE/ReplicateTextEffect", 2)
        end)
    end

    if replicateText then
        replicateText.OnClientEvent:Connect(function(data)
            local ok, typ = pcall(function() return data and data.TextData and data.TextData.EffectType end)
            if not ok or not typ then return end
            if autoFishingV5Enabled and typ == "Exclaim" then
                local head = player.Character and player.Character:FindFirstChild("Head")
                if head and data.Container == head then
                    print("🎣 V5 - Exclaim detected! Instant finish & recast...")
                    task.spawn(function()
                        -- best-effort: spam finishEvent a few times
                        for i = 1, 4 do
                            safeFire(finishEvent, true)
                            task.wait(0.004)
                        end

                        -- small wait then recast quickly (charge + request) to continue loop instantly
                        task.wait(0.03)
                        local ts = nil
                        pcall(function() ts = workspace:GetServerTimeNow() end)
                        if not ts then ts = tick() end
                        pcall(function() if rodRemote then rodRemote:InvokeServer(ts) end end)
                        task.wait(0.006)
                        pcall(function() if miniGameRemote then miniGameRemote:InvokeServer(-0.7499996, 1, ts) end end)
                    end)
                end
            end
        end)
        ...
        warn("[V5] ReplicateTextEffect not found - Exclaim detection disabled")
    end
end)
