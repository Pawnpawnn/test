...
local function ExecuteUltraBiteCycle()
    local catches = 0
    local debugMode = false -- Will be set from UI
    ...
    pcall(function()
        ...
        -- Step 3: Wait for bite detection (NEW METHOD)
        local waitStart = tick()
        local maxWait = 2.0 -- Wait max 2 seconds for bite
        ...
        if FishingState.IsBiting then
            if debugMode then print("[DEBUG] 🎣 BITE DETECTED! Catching...") end
            FishingState.IsBiting = false
            
            -- Method A: Use StartMini + FinishFish
            if StartMini and FinishFish then
                pcall(function()
                    StartMini:InvokeServer(-1.233184814453125, 0.9945034885633273)
                    if debugMode then print("[DEBUG] Started perfect minigame") end
                    task.wait(0.05)
                    FinishFish:FireServer()
                    if debugMode then print("[DEBUG] Finished fishing") end
                    catches = catches + 1
                end)
            end
            
            -- Method B: Direct FishCaught (backup)
            if FishCaught and catches == 0 then
                pcall(function()
                    FishCaught:FireServer()
                    catches = catches + 1
                    if debugMode then print("[DEBUG] Direct catch successful") end
                end)
            end
        else
            -- No bite detected, try instant catch anyway
            if debugMode then print("[DEBUG] No bite detected, trying instant catch...") end
        ...
        if debugMode then print("[DEBUG] Cycle complete. Catches:", catches) end
    end)
...
local function CreateUI()
    ...
    Rayfield:Notify({
        ...
        Content = "All systems loaded! Features: V1 Game Auto + Webhook + Event Teleport",
        Duration = 6,
        Image = 4483362458
    })
end
