...
-- Auto Fishing V5 Loop yang Cepat dan Efisien
local function autoFishingV5Loop()
    while autoFishingV5Enabled do
        pcall(function()
            fishingV5Active = true
            
            -- PHASE 1: Equip rod (sekali saja)
            if equipRemote then
                equipRemote:FireServer(1)
            end
            task.wait(0.1)

            -- PHASE 2: Charge rod (sekali saja)
            if ChargeRod then
                ChargeRod:InvokeServer(tick())
            end
            task.wait(0.1)

            -- PHASE 3: Start mini game (sekali saja)
            if StartMini then
            ...
            task.wait(0.5) -- Tunggu bentar biar game kebaca

            -- PHASE 4: Finish fishing - double tap untuk pasti
            if FinishFish then
                FinishFish:FireServer(true)
                task.wait(0.05)
                FinishFish:FireServer(true) -- Double tap
            end
            
        end)
        
        task.wait(0.3) -- Delay antar cycle
    end
...
local function StartFishingV5()
    ...
    pcall(function()
        print("🎣 FISHING V5 STARTED - Fast Mode")
...
task.spawn(function()
    ...
    if success and exclaimEvent then
            ...
            pcall(function()
                if autoFishingV5Enabled and data and data.TextData
                    ...
                    if head and data.Container == head then
                        print("🎣 Ikan strike! Auto reel...")
                        
                        -- Tarik ikan 2x untuk memastikan
                        task.wait(0.1)
                        if FinishFish then 
                            FinishFish:FireServer(true)
                            task.wait(0.05)
        ...
        print("✅ Exclaim detection activated")
    else
