...
local function startAutoFavorite()
    task.spawn(function()
        while autoFavoriteEnabled do
            pcall(function()
                -- Coba berbagai cara untuk mendapatkan data inventory
                local inventoryData = nil
                
                -- Cara 1: Coba melalui ReplicatedStorage
                local success1, result1 = pcall(function()
                    return ReplicatedStorage:FindFirstChild("Inventory") or 
                           ReplicatedStorage:FindFirstChild("PlayerData") or
                           ReplicatedStorage:FindFirstChild("Data")
                end)
                
                if success1 and result1 then
                    -- Logic untuk memproses inventory
                    updateStatus("⭐ Scanning Inventory...", Color3.fromRGB(255, 215, 0))
                    
                    -- Simulasi favorite items (ganti dengan logic sesuai game)
                    local favoriteCount = 0
                    
                    -- Contoh: Jika ada system events untuk favorite
                    local favoriteEvent = ReplicatedStorage:FindFirstChild("FavoriteItem") or
                                         ReplicatedStorage:FindFirstChild("ToggleFavorite")
                    
                    if favoriteEvent then
                        -- Logic untuk favorite items berdasarkan tier
                        -- Anda perlu menyesuaikan dengan API game yang sebenarnya
                        updateStatus("⭐ Auto Favorite: Searching High Tier Items", Color3.fromRGB(255, 215, 0))
                    else
                        updateStatus("❌ Favorite System Not Found", Color3.fromRGB(255, 100, 100))
                    end
                    
                else
                    -- Cara 2: Coba melalui Player Data
                    ...
                    if success2 and result2 then
                        updateStatus("⭐ Auto Favorite: Active (Method 2)", Color3.fromRGB(100, 255, 100))
                    else
                        updateStatus("❌ Cannot Access Inventory Data", Color3.fromRGB(255, 100, 100))
                    end
                end
            end)
            
            -- Delay antara eksekusi
            for i = 1, 30 do  -- Check every 0.5 seconds for 15 seconds total
